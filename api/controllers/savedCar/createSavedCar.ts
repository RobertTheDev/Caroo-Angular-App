...
import createSavedCarSchema from 'models/savedCar/createSavedCar.schema';
...
export default async function createSavedCar(req: Request, res: Response) {
  try {
    ...
    // Validate the body.
    const validation = await createSavedCarSchema.safeParseAsync(req.body);
    ...
    if (validation.success) {
      ...
      return res.status(StatusCodes.ACCEPTED).send({
        ...
        statusMessage: `Successfully created car request with id ${data.id}`,
        data,
      });
    } else {
    ...
    // Log the error.
    winstonLogger.error(`Error creating saved car:`, error);
